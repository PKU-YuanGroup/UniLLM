...
class JanusTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.multi_scale_lr is not None:
                ...
                multi_scale_lr_parameters = [
                    name for name, _ in optimized_parameters if "var"  in name  
                ]
                # print(lm_pos_parameters, '#####')
                decay_multi_scale_parameters = [name for name in multi_scale_lr_parameters if name in decay_parameters]
                nodecay_multi_scale_parameters = [name for name in multi_scale_lr_parameters if name not in decay_parameters]
                optimizer_grouped_parameters.extend([
                    {
            ...
            if self.args.aligner_lr is not None:
                projector_parameters = [name for name, _ in optimized_parameters if "aligner" in name and "gen_aligner" not in name]
            ...
            if self.args.vision_encoder_lr is not None:
                vision_encoder_parameters = [name for name, _ in optimized_parameters if "vision_model" in name and "gen_vision_model" not in name]
            ...
            if self.args.gen_aligner_lr is not None:
                gen_aligner_parameters = [name for name, _ in optimized_parameters if "gen_aligner" in name]
                decay_gen_aligner_parameters = [name for name in gen_aligner_parameters if name in decay_parameters]
                nodecay_gen_aligner_parameters = [name for name in gen_aligner_parameters if name not in decay_parameters]
                optimizer_grouped_parameters.extend([
                    {
            ...
            if self.args.gen_head_lr is not None:
                gen_head_parameters = [name for name, _ in optimized_parameters if "gen_head" in name]
                decay_gen_head_parameters = [name for name in gen_head_parameters if name in decay_parameters]
                nodecay_gen_head_parameters = [name for name in gen_head_parameters if name not in decay_parameters]
                optimizer_grouped_parameters.extend([
                    {
            ...
            if self.args.gen_embed_lr is not None:
                gen_embed_parameters = [name for name, _ in optimized_parameters if "gen_embed" in name]
                decay_gen_embed_parameters = [name for name in gen_embed_parameters if name in decay_parameters]
                nodecay_gen_embed_parameters = [name for name in gen_embed_parameters if name not in decay_parameters]
                optimizer_grouped_parameters.extend([
                    {
    ...
    #     if isinstance(outputs, dict) and "loss2" in outputs and "loss3" in outputs:
    #         loss2 = outputs["loss2"]
    #         loss3 = outputs["loss3"]
    #         self.log({
    #             "loss": loss.item(),
    ...
    #     loss2 = outputs.get("loss2", torch.tensor(0.0).to(total_loss.device))
    #     loss3 = outputs.get("loss3", torch.tensor(0.0).to(total_loss.device))
    #     output_dict = {
    #         "loss": total_loss,
    #         "loss2": loss2,
    #         "loss3": loss3
    ...
    #     return {
    #         "loss": loss.detach(),
    #         "loss2": outputs["loss2"].detach(),
    #         "loss3": outputs["loss3"].detach()
    #     }
    ...
    #         super(JanusTrainer, self)._save_checkpoint(model, trial, metrics)
    #     else:
    #         super(JanusTrainer, self)._save_checkpoint(model, trial, metrics)

    # def _save(self, output_dir: Optional[str] = None, state_dict=None):
    #     super(JanusTrainer, self)._save(output_dir, state_dict)